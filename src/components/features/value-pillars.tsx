...
export const ValuePillars = () => {
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
      {pillars.map((pillar, index) => <Card key={index} className="glass-card border-primary/20">
          <CardHeader className="pb-2">
            <div className="mb-3 p-2.5 w-fit rounded-lg bg-primary/20 flex items-center justify-center">
              {pillar.icon}
            </div>
            <CardTitle className="text-xl mx-px text-center">{pillar.title}</CardTitle>
            <CardDescription>{pillar.description}</CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2">
              {pillar.features.map((feature, idx) => <li key={idx} className="flex items-start gap-2">
